...
pub fn directives<'a>() -> impl Parser<TokenStream<'a>, Output=Item> {
    choice((
        choice((
            ...
            ident("proxy_keepalive")
                .with(many1(value()))
                .skip(semi()).map(Item::ProxyKeepAlive),
        ...
        choice((
            ...
            ident("proxy_cache_use_stale")
                .with(many1(value()))
                .skip(semi()).map(Item::ProxyCacheUseStale),
            ident("proxy_cache_revalidate").with(bool()).skip(semi())
                .map(Item::ProxyCacheRevalidate),
            ident("proxy_cache_lock").with(bool()).skip(semi())
                .map(Item::ProxyCacheLock),
            ident("proxy_cache_path").with(many1(value())).skip(semi())
                .map(Item::ProxyCachePath),
        ...
        choice((
            ident("proxy_ssl_name").with(value()).skip(semi())
                .map(Item::ProxySslName),
            ident("proxy_ssl_trusted_certificate").with(value()).skip(semi())
                .map(Item::ProxySslTrustedCertificate),
        )),
    ))
}
